...
TIMEOUT_S=${ETAAC_FULL_EMULATOR_TIMEOUT:-3600}
[[ $TIMEOUT_S =~ ^[0-9]+$ ]] && ((TIMEOUT_S>=1 && TIMEOUT_S<=3600)) || exit 2
...
mkdir -p "$OUT"
printf 'software-only, SDK full virtual topology, shader shire0/hart0, timeout=%s seconds\n' "$TIMEOUT_S" >"$OUT/scope.txt"
sha256sum "$ROOT/$KERNEL" "$ROOT/build-et/aac-full/host/full-emulator-run" "$ROOT/$INPUT" "$ROOT/$GOLD" >"$OUT/provenance.sha256"
rc=0
FF_ET_ALLOW_PCIE=0 FF_ET_SYSEMU=1 "$ROOT/et-tools/et-env" timeout "$TIMEOUT_S" env ETAAC_FULL_EMULATOR_GUARD=1 \
