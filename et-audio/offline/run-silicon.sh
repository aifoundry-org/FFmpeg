...
[[ $NAME =~ ^[a-zA-Z0-9_-]+$ && $TASKS =~ ^[0-9]+$ && $FRAMES =~ ^[0-9]+$ && $CHUNK =~ ^[0-9]+$ && ($HARTS == 1 || $HARTS == 32 || $HARTS == 64) && ($OP == 0 || $OP == 1) && ($MODE == 0 || $MODE == 1) ]] || exit 2
((TASKS>=1 && TASKS<=64 && FRAMES>=1 && FRAMES<=512 && CHUNK>=1 && CHUNK<=FRAMES && FRAMES%CHUNK==0)) || exit 2
[[ $CAPTURE == build-et/aac-offline/captures/* && $CAPTURE != *..* ]] || exit 2
...
CHECK_CAPTURE
OUT="$ROOT/build-et/aac-offline/silicon/$NAME"
[[ ! -e $OUT ]] || { echo "Refusing to overwrite $OUT" >&2; exit 2; }
mkdir -p "$OUT"
BLOCK="$ROOT/build-et/aac-offline/RECOVERY_REQUIRED"
for marker in "$BLOCK" "$ROOT/build-et/aac-prototype/RECOVERY_REQUIRED" "$ROOT/build-et/silicon/RECOVERY_REQUIRED" \
...
KERNEL=${ETAAC_KERNEL:-build-et/aac-offline/device/et_aac_offline.elf}
RUNNER=build-et/aac-offline/host/offline-run
[[ $KERNEL == build-et/aac-offline/* && $KERNEL != *..* ]] || exit 2
...
printf 'channels=%s\nharts=%s\nframes=%s\nchunk=%s\nmode=%s\noperation=%s\ncpu_affinity=0\n' "$TASKS" "$HARTS" "$FRAMES" "$CHUNK" "$MODE" "$OP" >"$OUT/config.txt"
active=0
...
"$ROOT/et-tools/et-env" timeout 180 taskset -c 0 env ETAAC_HW_GUARD=1 \
 "$RUNNER" "/work/$KERNEL" "/work/$CAPTURE" "$TASKS" "$HARTS" "$FRAMES" "$CHUNK" "$MODE" "$OP" \
...
python3 - "$OUT/run.jsonl" "$TASKS" "$FRAMES" "$CHUNK" <<'PY'
import json,sys
